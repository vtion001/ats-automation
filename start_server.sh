...
nohup python3 -m uvicorn server.ai_server:app --host 0.0.0.0 --port 8000 > /tmp/ags_server.log 2>&1 &
...
# Check if running
if curl -s http://localhost:8000/health > /dev/null 2>&1; then
    echo "✅ AGS AI Server started on http://localhost:8000"
    echo "📋 Health: http://localhost:8000/health"
    echo "📋 API: http://localhost:8000/api/analyze"
else
