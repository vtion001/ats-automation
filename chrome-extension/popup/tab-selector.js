...
async function stopRecording() {
    ...
    try {
        const response = await chrome.runtime.sendMessage({ type: 'STOP_TAB_CAPTURE' });
        isRecording = false;
...
async function processAndAnalyze() {
    updateStatus('Fetching audio...', 'stopped');
    
    try {
        const audioResponse = await chrome.runtime.sendMessage({ type: 'GET_CAPTURED_AUDIO' });
        
        if (!audioResponse || !audioResponse.success || !audioResponse.audio) {
            updateStatus('No audio data found', 'stopped');
            setWorkingUI(false);
            showManualTranscribe();
            return;
        }
        ...
        const duration = audioResponse.duration
            ? ` (${formatDuration(audioResponse.duration)})`
            : '';
...
function showManualTranscribe(existingText = '') {
    ...
    container.style.cssText = 'margin-top:12px;padding:12px;background:rgba(255,255,255,0.08);border-radius:8px;';
    container.innerHTML = `
        <p style="font-size:12px;margin-bottom:8px;opacity:0.8;">Paste the call transcript below:</p>
        <textarea id="manualTranscriptText" placeholder="Paste transcript here..." rows="5" style="width:100%;border-radius:6px;border:none;padding:8px;font-size:13px;box-sizing:border-box;resize:vertical;background:rgba(255,255,255,0.1);color:white;"></textarea>
...
function displayResults(result) {
    ...
    container.style.cssText = 'margin-top:12px;padding:0;background:rgba(255,255,255,0.05);border-radius:8px;overflow:hidden;';
    
    container.innerHTML = `
        <div style="padding:12px 16px;background:rgba(255,255,255,0.05);border-bottom:1px solid rgba(255,255,255,0.1);">
            <div style="display:flex;align-items:center;justify-content:space-between;">
                <div style="display:flex;align-items:center;gap:10px;">
                    ...
                    <div>
                        <div style="font-weight:600;font-size:14px;">${scoreClass === 'hot' ? 'Hot Lead' : scoreClass === 'warm' ? 'Warm Lead' : 'Cold Lead'}</div>
                        <div style="font-size:12px;opacity:0.7;">${sentiment.charAt(0).toUpperCase() + sentiment.slice(1)} sentiment</div>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div style="padding:12px 16px;">
            ${tags.length ? `<div style="margin-bottom:10px;">
                <div style="font-size:11px;text-transform:uppercase;opacity:0.6;margin-bottom:4px;">Tags</div>
                <div style="display:flex;flex-wrap:wrap;gap:4px;">
                    ${tags.map(tag => `<span style="padding:3px 8px;background:rgba(49,130,206,0.3);border-radius:12px;font-size:11px;">${tag}</span>`).join('')}
                </div>
            </div>` : ''}
            ...
            ${summary ? `<div style="margin-bottom:10px;">
                <div style="font-size:11px;text-transform:uppercase;opacity:0.6;margin-bottom:4px;">Summary</div>
                <div style="font-size:13px;line-height:1.4;">${summary}</div>
            </div>` : ''}
            
            ${transcription ? `<div style="margin-bottom:10px;">
                <div style="font-size:11px;text-transform:uppercase;opacity:0.6;margin-bottom:4px;">Transcript</div>
                <div style="font-size:12px;background:rgba(0,0,0,0.15);padding:8px;border-radius:6px;max-height:120px;overflow-y:auto;line-height:1.4;white-space:pre-wrap;">${transcription}</div>
            </div>` : ''}
            
            ${notes ? `<div style="margin-bottom:10px;">
                <div style="font-size:11px;text-transform:uppercase;opacity:0.6;margin-bottom:4px;">Salesforce Notes</div>
                <div style="font-size:12px;background:rgba(30,58,95,0.3);padding:8px;border-radius:6px;line-height:1.4;">${notes}</div>
                <button class="btn-copy-notes" style="margin-top:6px;background:rgba(255,255,255,0.1);border:none;color:white;padding:5px 10px;border-radius:4px;cursor:pointer;font-size:12px;">
            ...
            <div style="font-size:12px;opacity:0.6;text-align:center;padding-top:8px;border-top:1px solid rgba(255,255,255,0.05);">
